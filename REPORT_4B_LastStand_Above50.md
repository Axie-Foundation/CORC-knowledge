# Report 4B — Last Stand: New Bars Table and Trigger Formula (Variant B: Boost Above Morale 50)
...
2. Change the Last Stand activation formula so triggering is biased upward at high morale, **with the boost beginning above morale 50 and scaling linearly to +30% at morale 64**.

Variant A (boost beginning above morale 54) is covered in a separate report.
...
## 3. Change 2 — The trigger formula (Variant B: boost above morale 50)

The user wants Last Stand to "trigger 20% more often" above morale 50, scaling linearly to "+30% at morale 64". Because the existing trigger is deterministic, the only well-defined reading of that is: **the effective morale used in the inequality is scaled up by +20–30% in the chosen band**.
...
boost%(m) =
    0                                                     if m ≤ 50
    20 + (m − 51) × (30 − 20) / (64 − 51)                 if 50 < m ≤ 64
    30                                                    if m > 64
```

This is a linear interpolation that hits +20% at the first morale point above 50 (i.e. morale 51) and +30% at morale 64, holding at +30% above 64.
...
| Morale | boost% | effective morale | Survives overkill up to |
|---|---|---|---|
| 50 | 0% | 50 | 50% of HP |
| 51 | 20.0% | 61.2 | 61.2% of HP |
| 54 | ~22.3% | 66.0 | 66.0% of HP |
| 55 | ~23.1% | 67.7 | 67.7% of HP |
| 60 | ~26.9% | 76.2 | 76.2% of HP |
| 64 | 30.0% | 83.2 | 83.2% of HP |
...
- **Variant B new:** boost%(60) ≈ 26.9%. effective_morale ≈ 76.2. Check: `75 × 100 ≤ 100 × 76.2` → `7500 ≤ 7620` → true → **Last Stand triggers**.

A second example unique to Variant B — an Axie at morale 52 (Variant A would not buff this Axie at all):

100 HP, 52 morale, takes 158 damage (58 overkill).

- **Current formula:** `58 × 100 ≤ 100 × 52` → `5800 ≤ 5200` → false → dies.
- **Variant B new:** boost%(52) ≈ 20.8%. effective_morale ≈ 62.8. Check: `58 × 100 ≤ 100 × 62.8` → `5800 ≤ 6280` → true → **Last Stand triggers**.
- **Variant A would not save this Axie** (it would still die under Variant A because morale 52 is below the threshold). This is the key behavioural difference between A and B.

Combined with the new bars-table grant from §2, this Axie also gets 4 bars (morale 50–54 band) instead of dying outright.
...
pub fn can_last_stand(&self, morale: i32, decrement: i32, guarantee_last_stand: bool) -> bool {
    ...
    let effective_morale = apply_morale_boost_b(morale);
    (decrement - self.hp) as i64 * 100 <= self.hp as i64 * effective_morale as i64
}

/// Variant B: +20% at morale 51, scaling linearly to +30% at morale 64.
fn apply_morale_boost_b(morale: i32) -> i32 {
    if morale <= 50 {
        return morale;
    }
    let m = morale.min(64);
    // boost_x100 ranges from 20 (at m=51) to 30 (at m=64). Integer math: ((m-51) * 10) / 13.
    let boost_x100: i32 = 20 + ((m - 51) * 10) / 13;
...
- The integer-math version is exact at the endpoints (`boost_x100 = 20` at morale 51, `boost_x100 = 30` at morale 64) and is monotone everywhere in between, with at most 1-percentage-point rounding error at intermediate morales. If exact floating-point boost is needed, use `f64` arithmetic and round at the end.
...
## 4. Why Variant B over Variant A?

Variant B is the **more aggressive** choice. It buffs not only the genuinely-high-morale Axies (Variant A's target) but also the morale 51–54 band — where new bars per the user's table is 4. This makes "morale stacking" attractive across more team archetypes, not just extreme high-morale builds.

**Behavioural difference between Variant A and Variant B:**
- An Axie at morale 52 receives **no boost in Variant A**, but receives **+20% boost in Variant B**.
- An Axie at morale 60 receives **+25.6% in Variant A**, **+26.9% in Variant B** — nearly identical.
- Both variants reach +30% at morale 64.

If the goal is "shake up the meta so morale becomes the primary stat to invest in for a broader range of team builds", Variant B does that more strongly. If the goal is the narrower "reward going from competitive to high morale", Variant A is enough.

For Variant A, see Report 4A.
...
2. `state.rs:247–251` — replace `can_last_stand` with the Variant B body from §3.4, and add the `apply_morale_boost_b` helper alongside.
...
4. Add unit tests for `get_last_stand_turns` covering boundary morales (34/35, 39/40, 44/45, 49/50, 54/55, 59/60, 64/65) and for `can_last_stand` covering the boost boundaries (50/51) and the morale-64 ceiling.
...
4. **The morale-50 → morale-51 cliff** — Variant B introduces a step change in trigger behaviour at exactly the boundary where bars also tick up (50–54 → 4 bars, 55–59 → 5 bars). The trigger boost jumps from 0% to +20% in a single morale point. If a smoother ramp is desired, the boost could start at 0% at morale 50 and ramp linearly — but that's a different formula from the one requested.
